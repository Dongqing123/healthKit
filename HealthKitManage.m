...
- (void)getStepCount:(void(^)(double value, NSError *error))completion withStartDate:(NSDate*)startDate andEndDate:(NSDate*)endDate
{
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc] initWithSampleType:stepType predicate:[HealthKitManage predicateForSamplesTodayWithStartDate:startDate andEndDate:endDate] limit:HKObjectQueryNoLimit sortDescriptors:@[timeSortDescriptor] resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
        if(error)
        {
            completion(0,error);
        }
 ...
 *  @return 时间段
 */
+ (NSPredicate *)predicateForSamplesTodayWithStartDate:(NSDate*)startDate andEndDate:(NSDate*)endDate {
...
//    startDate = [self changeDate:startDate];
//    endDate = [self changeDate:endDate];
